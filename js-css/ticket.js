...
function pushTicket(){
    ...
    if (produits.length==0){
        ...
        for (let i=1 ; i<produits.length+1 ; i++){
            ...
            let prix = prixP.replace(/\bTND\b/,'');
            qtte.innerHTML = quantite
            data.innerHTML = label+' ('+prix+' TND)'
            total.innerHTML = quantite * prix + ' TND'
            gT += quantite * prix
        ...
        let SOUM = document.createElement('input')
        SOUM.setAttribute('type','text')
        SOUM.setAttribute('name','global-total')
        SOUM.setAttribute('value',gT)
        form3.appendChild(SOUM)
